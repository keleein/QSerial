        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
          ...
          {running && (
            <div>
              ...
              <div
                className="border border-border/50 rounded-lg bg-background/50 overflow-y-auto max-h-40"
                style={{ minHeight: transfers.length > 0 ? 'auto' : '56px' }}
              >
                {transfers.length === 0 ? (
                    ...
                    {transfers.map((transfer) => (
                      <div key={transfer.id} className="p-2.5 text-sm">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span
                            className={`flex-shrink-0 text-xs ${getStatusColor(transfer.status)}`}
                          >
