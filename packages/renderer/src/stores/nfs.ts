  ...
  persist(
    (set, get) => ({
      ...
      startServer: async () => {
        ...
        try {
          await window.qserial.nfs.start(config.exportDir, config.allowedClients, config.options);
      ...
      loadStatus: async () => {
        try {
          ...
          if (status.running) {
            ...
            if (currentRunning) {
              console.log(
                '[NFS] loadStatus: main process reports not running, syncing state to false'
              );
            }
...
export function initNfsListeners(): void {
  ...
  window.qserial.nfs.onStatusChange((event) => {
    ...
    console.log('[NFS] onStatusChange:', JSON.stringify(event), 'state:', {
